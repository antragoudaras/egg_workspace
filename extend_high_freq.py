...
parser.add_argument("--save-dataset", type=str, default='./dataset_extend_high_freq.xlsx', help="The path where the generated dataset will be stored")
...
d = {'n_filters_time': [40], 'filter_time_length': [25], 'n_filters_spat': [40], 'pool_time_length': [75], 'pool_time_stride': [15], 'drop_prob': [0.5], 'learning_rate_range': [0.0625], 'decay_range': [0], 'accuracy': [71.913580]}
...
for j in range(100):
    ...
    for i in range(9):
        ...
        # in the dataset.
        print("SUBJECT", i+1, "ARCHITECTURE", j+1)
        clf.fit(train_set, y=None, epochs=n_epochs)

        # Extract loss and accuracy values for plotting from history object
        results_columns = ['train_loss', 'valid_loss', 'train_accuracy', 'valid_accuracy']
        df22 = pd.DataFrame(clf.history[:, results_columns], columns=results_columns,
                          index=clf.history[:, 'epoch'])

        avgList.append(100*df22.valid_accuracy[n_epochs])
