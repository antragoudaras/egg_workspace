...
parser.add_argument("--interested-patient", type=int, default=4, help="Patient that was left out during zero-shot/contextual training for PRIME ")
...
accuracy_of_interest = [0.0 for _ in range(num_of_archs)]
df['accuracy'] = accuracy
df['accuracy_of_interest'] = accuracy_of_interest
num_of_subjects= 9
...
for j in range(num_of_archs):
	...
	for i in range(num_of_subjects):
		...
		if subject_id == args.interested_patient:
			df.at[j, 'accuracy_of_interest'] = 100 * df_scores.valid_accuracy[n_epochs]
...
df.to_excel(f"{prefix}_high_freq_ground_truth_results_contextual_BCI_2a_EEG_leave_one_out.xlsx")