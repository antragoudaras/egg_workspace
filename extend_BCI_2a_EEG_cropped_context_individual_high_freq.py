...
parser = argparse.ArgumentParser("Generating EEG signals for BCI 2a Dataset parser High Freq Contextual")
...
d = {'n_filters_time': [40 for _ in range(subjects_num)], 'filter_time_length': [25 for _ in range(subjects_num)], 'n_filters_spat': [40 for _ in range(subjects_num)], 'pool_time_length': [75 for _ in range(subjects_num)], 'pool_time_stride': [15 for _ in range(subjects_num)], 'drop_prob': [0.5 for _ in range(subjects_num)], 'learning_rate_range': [0.0625 for _ in range(subjects_num)], 'decay_range': [0 for _ in range(subjects_num)], 'accuracy': [80.9027777777778, 45.4861111111111, 88.5416666666667, 68.75, 61.8055555555556, 52.4305555555556, 90.9722222222222, 82.2916666666667, 79.1666666666667], 'subject_id': [int(idx+1) for idx in range(subjects_num)]}
...
for j in range(num_arch_explored):
    for i in range(subjects_num):
        ...
        # lr = df.iloc[(j+1)*subjects_num+i]['learning_rate_range'] * 0.01 #rember to test it with 0.01 multiplyer
