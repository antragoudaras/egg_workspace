...
DATASET_PATH="$SRC_DIR"/contextual_EEG_low_freq_positive_COM_optimized_params_november/"$JOB_NAME"
echo "$DATASET_PATH"

sbatch --job-name "$JOB_NAME" "$SRC_DIR"/test_contextual_prime_low_freq_BCI_2a_EEG.sbatch --load-dataset "$DATASET_PATH"


echo "Testing of 25 contextual PRIME generated archs. of BCI EEG low-freq"