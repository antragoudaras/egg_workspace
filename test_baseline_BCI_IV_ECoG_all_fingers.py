...
for finger in range(5):
    ...
    for i in range(3):
        ...
        #
        train_set.target_transform = lambda x: x[finger: finger+1]
        valid_set.target_transform = lambda x: x[finger: finger+1]
        test_set.target_transform = lambda x: x[finger: finger+1]
    ...
    print('Finger{}: Average r2 valid list : {} across {} subjects'.format(finger+1,accuracy_individual_list,i+1))
    print('Finger{}: Average r2-valid score: {} across {} subjects'.format(finger+1,np.array(accuracy_individual_list).mean(),i+1))
    average_final_list.append(np.asscalar(np.array(accuracy_individual_list).mean()))

print('Final list with r2 average scores from each finger: {}'.format(average_final_list))
print('Average r2-valid score: {} of fingers'.format(np.array(average_final_list).mean()))